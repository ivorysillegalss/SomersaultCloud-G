...
func (b baseMessageHandler) InitMessageQueue(args ...any) {
	for _, argAny := range args {
		...
		if err != nil {
			log2.GetJsonLogger().WithFields("create producer channel err", err.Error()).Fatal(mq.MqPublishErr + ": " + arg.QueueName)
		} else {
			log2.GetTextLogger().Info("create publish channel success " + ": " + arg.QueueName)
		}
		// 创建消费者 Channel
		consCh, err := b.conn.NewChannel()
		if err != nil {
			log2.GetJsonLogger().WithFields("create consumer channel err", err.Error()).Fatal(mq.MqConsumeErr + ": " + arg.QueueName)
		} else {
			log2.GetTextLogger().Info("create consumer channel success" + ": " + arg.QueueName)
		}

		if err := prodCh.ExchangeDeclare(arg.ExchangeName, "direct"); err != nil {
			log2.GetJsonLogger().WithFields("create exchange err", err.Error()).Fatal(mq.MqPublishErr + ": " + arg.ExchangeName)
		} else {
			log2.GetTextLogger().Info("create exchange success")
		}
		...
		if arg.ExistDeadLetterQueue {
			if err := prodCh.QueueDeclareDeadLetter(arg.QueueName, arg.DeadLetterExchange, arg.DeadLetterRoutingKey); err != nil {
				log2.GetJsonLogger().WithFields("create queue err:", err.Error()).Fatal(mq.MqPublishErr)
			}
		} else {
		...
		log2.GetTextLogger().Info("create queue success")

		if err := prodCh.QueueBind(arg.QueueName, arg.KeyName, arg.ExchangeName); err != nil {
			log2.GetJsonLogger().WithFields("bind queue err:", err.Error()).Fatal(mq.MqPublishErr)
		} else {
			log2.GetTextLogger().Info("bind queue success")
		}
...
func (b baseMessageHandler) PublishMessage(queueName string, data []byte) {
	...
	go func() {
		if err := args.ProducerChannel.Publish(args.ExchangeName, args.KeyName, data); err != nil {
			log2.GetJsonLogger().WithFields("publish msg err", err.Error()).Fatal(mq.MqPublishErr)
		}
...
func (b baseMessageHandler) ConsumeMessage(queueName string, handler func([]byte) error) {
	go func() {
		...
		if config, exists := messageQueueConfigs[queueName]; exists && config.ConsumerChannel != nil {
			...
			if err != nil {
				log2.GetTextLogger().WithFields("new mq channel err", err.Error()).Fatal(mq.MqConsumeErr)
				return
			}
		...
		if err := b.conn.NewConsumer(ch, queueName, handler); err != nil {
			log2.GetJsonLogger().WithFields("consume err", err.Error()).Fatal(mq.MqConsumeErr)
		}
		log2.GetTextLogger().Info("consume message success")
	}()
}
